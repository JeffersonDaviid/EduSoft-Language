...
export const Progress = () => {
	...
	const playedTypes = new Set(
		progress.history.filter((row) => GAME_TYPES.includes(row.game)).map((row) => row.game)
	);
	...
	const getPracticeStreak = () => {
		const dates = Array.from(
			new Set(progress.history.map((row) => new Date(row.playedAt).toDateString()))
						...
						<div className='w-full max-w-[960px] flex flex-col gap-6'>
							<div className='flex flex-col gap-3'>
								...
								<p className='text-sm md:text-base text-[#4C7490]' tabIndex={0}>
									...
									overall progress, see your average scores for each game type, check your
									daily practice streak, and get friendly tips to helper you improve even
									more.
								</p>
							</div>
								...
								{GAME_TYPES.map((type) => {
									...
									return (
										...
										>
											...
											<p className='text-2xl font-bold text-blue-700' tabIndex={0}>
												{avg !== null ? (
													`${avg}/100`
												) : (
							...
							<section className='w-full flex flex-row items-center justify-start gap-4 px-4 py-2'>
								<div className='bg-white rounded-lg shadow p-4 flex flex-col items-center min-w-[120px]'>
									<span className='text-3xl font-bold text-emerald-600' tabIndex={0}>
										{streak}
									</span>
								...
								{minAvgType && (
									...
									>
										...
										could use a little extra practice! Give it another try to boost your
										overall progress. 🚀
									</div>
								)}
							</section>
